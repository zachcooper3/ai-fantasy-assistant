...
# Output budget. Has to comfortably fit the whole JSON response: strategy,
# confidence, the single `main` pick, the MUST-EVALUATE `considered` verdicts,
# and the alerts array. `best_available`/`needs`/`depth` are NOT generated by
# the model at all — see RecommendationResult's docstring — so this is
# sized for a much smaller response than the old recommendation+alternatives
# shape carried. Left at the old ceiling rather than lowered: max_tokens is a
# cap, not a cost (Anthropic bills actual output tokens either way), and a
# generous ceiling costs nothing while a too-tight one silently truncates the
...
# Models where Anthropic's "adaptive thinking" runs by default and we
# explicitly turn it off (`thinking: {"type": "disabled"}`), rather than
# leaving it on at some effort level. Coop's call, 2026-08-10: for a single
# structured pick recommendation on a live draft clock, the reasoning
# thinking adds isn't worth its cost — the prompt already spells out the
# analysis framework (ADP tiers, scarcity, opportunity cost), so thinking's
# main value-add (working through an open-ended problem) has less to do
# here than on e.g. a hard coding task. Two earlier approaches were tried
# and superseded by this one: leaving thinking on at the API default
# (uncapped `high` effort — expensive) and leaving thinking on at
# `effort: "low"` (cheaper but still nonzero thinking cost per call). This
# is simpler than both: no thinking tokens, no effort tuning to reason
...
def _completion_kwargs(model: str) -> dict:
    ...
    Explicitly disables thinking for _THINKING_DISABLED_MODELS — see that
    constant's docstring for why. max_tokens is always _MAX_RESPONSE_TOKENS:
    with thinking off there's no invisible thinking spend to leave headroom
    for.
    """
...
class PickSuggestion:
    ...
    adp: float
    reasoning: str
    # Which section(s) of RecommendationResult this player appears in — e.g.
    # ["main"], ["best_available"], or ["main", "best_available"] when the
    # same player wins both. Overlap is kept, not deduped away: see
    # RecommendationResult's docstring for why a player being both the best
    # value AND the main pick is meaningful information, not a redundancy to
    # hide. Never empty on a real suggestion.
...
class RecommendationResult:
    ...
    # Empty for the ADP fallback, which evaluates nothing. Not rendered in
    # the UI — kept purely as the server-side omission safeguard (see
    # _shortlist's docstring for the DST incident this exists to prevent) —
    # so its cost is bounded output tokens, never bandwidth or screen space.
...
class _GapInfo:
    ...
    late_reserved: int
    skill_rounds_remaining: int
    kickers_available: bool


def _compute_gap_info(ctx: RecommendationContext) -> _GapInfo:
    """
    Single source of truth for the starting-lineup gap split, the DST/K
    roster-tax gate, and the "how many usable rounds are left" math.

    Shared by _build_prompt's prose (the "Still owed"/"URGENT" lines the
    model reads) AND the deterministic best_available/needs/depth sections
    (see RecommendationResult) that get shown on screen without the model
    ever seeing them — so the two can never disagree about whether DST is
    due yet. Two independent implementations of that same question is
    exactly how the DST-never-recommended bug happened the first time: the
    prompt said "draft now" while a separate, unrelated code path never
    required the model to act on it. One computation, read from both places.
    """
    ...
    # Counts the current pick — see _build_prompt's own note on why the old
    # (total_rounds - round_number) expression under-counted by one.
    ...
    return _GapInfo(
        ...
        due_late_gaps=due_late_gaps, rounds_remaining=rounds_remaining,
        late_reserved=late_reserved, skill_rounds_remaining=skill_rounds_remaining,
        kickers_available=kickers_available,
    )
...
#
# These build RecommendationResult's non-`main` sections directly from
# ctx.top_available and the gap math in _compute_gap_info, with NO model
# call involved. See RecommendationResult's docstring for why: "cheapest by
# ADP" and "cheapest at an open position" are lookups the model would
# otherwise be asked to re-derive and write out in full sentences, on every
# single pick of every draft, for no analytical gain over just computing
# them.
# ---------------------------------------------------------------------------

# Tags applied to PickSuggestion.tags, marking which RecommendationResult
# section(s) a player appears in. A player can carry more than one — see
# _tag_overlaps.
_TAG_MAIN = "main"
...
# rules correctly treat a QB2/TE2 as pure roster-spot waste once the
# starter slot is filled, which is exactly right until the starting lineup
...
# matches the order used everywhere else in the prompt (_slot_order in
# _build_prompt) so the neediest position by convention, not alphabetical
# accident, gets the scarce slots first.
_POSITION_PRIORITY = ("QB", "RB", "WR", "TE", "DST", "K")
...
def _needs(
    ...
    """
    The realistic slate at your highest-priority open starting slot(s):
    cheapest AND best-VOR at that position — the same pair
    _board_for_prompt already force-includes on the board, reused here
    rather than re-derived (see that function's docstring for why two
    axes, not one, are needed when they disagree).

    Walks positions in _POSITION_PRIORITY order and stops at
    _NEEDS_SLOTS, so with multiple gaps open the neediest position (by the
    ...
    positions = sorted(
        gaps,
        key=lambda p: _POSITION_PRIORITY.index(p) if p in _POSITION_PRIORITY else len(_POSITION_PRIORITY),
    )

    seen: set[int] = set()
    out: list[PickSuggestion] = []

    def _add(p: dict, reason: str) -> None:
        if p["id"] in seen:
            return
        seen.add(p["id"])
        out.append(PickSuggestion(
            player_id=p["id"], player_name=p["name"], position=p["position"],
            adp=p["adp"], reasoning=reason, tags=[_TAG_NEEDS],
            survival=_survival_code(p["adp"], ctx.my_following_pick_number),
        ))

    for pos in positions:
        if len(out) >= _NEEDS_SLOTS:
            break
        at_pos = sorted(
        ...
        _add(at_pos[0], f"Fills your open {pos} slot — cheapest available.")
        if len(out) >= _NEEDS_SLOTS:
            break
        ...
        if scored:
            best_vor_player = max(scored, key=lambda t: t[0])[1]
            _add(best_vor_player, f"Fills your open {pos} slot — best value there.")
...
def _tag_overlaps(
    ...
    for tag, items in groups.items():
        for item in items:
            tags_by_id.setdefault(item.player_id, []).append(tag)
    for items in groups.values():
        for item in items:
            item.tags = list(tags_by_id[item.player_id])
...
def _build_sections(
    ...
    "depth" means for the same draft state.
    """
...
def _build_prompt(ctx: RecommendationContext) -> str:
    ...
    # once got recommended across a full draft as a result). See
    # _compute_gap_info's docstring for why this one computation feeds both
    # this prose AND the deterministic best_available/needs/depth sections.
    gap_info = _compute_gap_info(ctx)
    kickers_available = gap_info.kickers_available
    lineup = gap_info.lineup
    late_gaps = gap_info.late_gaps
    skill_gaps = gap_info.skill_gaps
    rounds_remaining = gap_info.rounds_remaining
    late_reserved = gap_info.late_reserved
    skill_rounds_remaining = gap_info.skill_rounds_remaining
    due_late_gaps = gap_info.due_late_gaps
    ...
    # due_late_gaps (from gap_info) is threaded into _board_for_prompt/
    # _shortlist below so a DST/K that's due can't be gated by text alone —
    # see _shortlist's docstring for the live failure where the prompt said
    ...
    lines += [
        ...
        "specific. THIS EXCEPTION IS FOR CONTEXT, NOT NUMBERS: every stat, percentage, "
        "or per-game figure you cite (PPG, snap %, target share, Y/tgt, OPP, and the "
        "rest) must be copied from a labeled figure elsewhere in this prompt, exactly "
        "as RULE 10 requires for `main` — general knowledge earns you a reason, never "
        "a number. If you want to cite usage or production and don't have the exact "
        "figure in front of you, describe it qualitatively instead of stating one. "
        "Always name someone: if nothing clearly stands out, say so in "
...
def _build_system_prompt(scoring_format: str = "ppr") -> str:
    ...
    return (
        ...
        "trace to something in the prompt. Numbers are the strictest case, because a "
        "wrong number reads as confidently as a right one: any stat you state — PPG, "
        "target share, snap %, Y/tgt, YAC, a percentage, a count — must be COPIED from "
        "a labeled figure already in this prompt, never computed, rounded from memory, "
        "or estimated from other numbers you do see. If you don't have the exact "
        "figure, say it in words ('an elite snap share', 'well below his tier') "
        "instead of writing a number you're not reading off the page. You have no "
        "bye-week data, no depth-chart narrative beyond what is shown, no 2026 "
        "projections, and no injury news past what appears in Player News & Analysis "
        "— so do not reason about bye-week conflicts, stacking, coaching changes, "
        "training-camp reports, or contract situations. If the deciding factor "
        "genuinely isn't in the prompt, say the call is close and set `confidence` to "
        "low. ONE NAMED EXCEPTION: the `opportunity` field asked for in the Task "
        "section below is explicitly allowed to reason about coaching/scheme changes, "
        "contract/depth-chart situations, and general football knowledge this rule "
        "otherwise forbids — see its instructions for the guardrails that apply there "
        "instead. That exception covers QUALITATIVE context only. The numbers rule "
        "above is NOT relaxed by it: every stat named in `opportunity.reasoning` must "
        "still be copied from a labeled figure in this prompt, exactly like `main`. "
        "General knowledge can supply a reason; it can never supply a statistic. This "
        "exception is otherwise scoped to that one field; `main` and everything else "
        "in your response still follow RULE 10 as written."
    )
...
def _parse_response(raw: str, ctx: RecommendationContext) -> RecommendationResult | None:
    ...
    # Every *factual* field on the pick comes from the canonical player row,
    # never from Claude's JSON — see _pick_from, which both this and the
    # streaming path go through.
    main = _pick_from(rec, ctx)
    if main is None:
        return None
    ...
    # verdicts or the reasoning was considered, and the guard stays quiet
    # about him. (Alternatives used to widen this string too — they're gone,
    # replaced by the deterministic best_available/needs/depth sections
    # below, which were never model output the guard needed to read.)
    engaged = " ".join([*considered, main.reasoning])
    dominance = _dominance_alert(main, ctx, engaged)
    if dominance:
        logger.info(
            "Dominated pick: %s was recommended over a same-position player with "
            "better ADP and higher VOR.", main.player_name,
        )
...
def _fallback(
    ...
    # Same deterministic sections a live recommendation gets — see
    # _build_sections. The fallback's `main` is routinely identical to
...
class AIService:
    ...
    async def recommend_stream(self, ctx: RecommendationContext):
        ...
        changes is when the answer becomes visible. Generation is
        sequential, so nothing appears until it finishes, and `main` is
        near the front of the schema (right after strategy/confidence), so
        the pick itself exists well before the `considered` verdicts and
        `alerts` that follow it are done streaming.
        ...
        try:
            async with self._client.messages.stream(
                **_completion_kwargs(self._model),
                ...
                async for chunk in stream.text_stream:
                    ...
                    # would look for has plausibly closed. "considered" is
                    # the key immediately after "main" in the schema, so its
                    # presence means main's object has had a chance to close.
                    if "considered" not in buffer and buffer.count("}") < 1:
                        continue
                    rec = _extract_complete_object(_restore_prefill(buffer), "main")
                    if rec:
            ...
            if getattr(final_message, "stop_reason", None) == "max_tokens":
                # Thinking is disabled for every model in
                # _THINKING_DISABLED_MODELS (see that constant), so this
                # ceiling is spent entirely on the visible JSON now — no
                # invisible thinking spend to blame. A genuine signal to
                # raise _MAX_RESPONSE_TOKENS or tighten the response shape.
                logger.warning(
                    "Claude's streamed response hit the %d-token ceiling and "
                    "was truncated — the JSON will not parse. Raise "
                    "_MAX_RESPONSE_TOKENS or tighten the requested response "
                    "shape.",
    ...
    async def recommend(self, ctx: RecommendationContext) -> RecommendationResult:
        ...
        try:
            ...
            if getattr(response, "stop_reason", None) == "max_tokens":
                ...
                # raise _MAX_RESPONSE_TOKENS or tighten the response shape.
                logger.warning(
                    "Claude response hit the %d-token ceiling and was "
                    "truncated — the JSON will not parse. Raise "
                    "_MAX_RESPONSE_TOKENS or tighten the requested response "
                    "shape.",
                    _MAX_RESPONSE_TOKENS,
                )
